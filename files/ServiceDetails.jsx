...
import { GIG_DETAILS, ISLAND_META } from '../../data/registry';
...
function HireConfirmation({ gig, pkg, onConfirm, onCancel }) {
  ...
  const handlePay = () => {
    setStep('processing');
    setTimeout(() => {
      // ── Muscle test: hiring awards +50 XP to the live store ──────────────
      addXP(50);
      setStep('success');
    }, 1800);
  };
      ...
      <div style={confirmStyles.overlay} onClick={onCancel}>
        <div style={{ ...confirmStyles.box, maxWidth: 380 }} onClick={e => e.stopPropagation()}>
          <div style={{ textAlign: 'center', padding: '10px 0 20px' }}>
            ...
            <h3 style={{ fontFamily: 'var(--font-display,"Playfair Display",serif)', fontSize: '1.3rem', color: '#e8f0eb', marginBottom: 8 }}>
              Order Placed!
            </h3>
            <p style={{ color: '#a3b899', fontSize: '0.85rem', lineHeight: 1.6 }}>
              Your order with <strong style={{ color: '#52b788' }}>{gig.sellerName}</strong> has been confirmed.
              You've earned <strong style={{ color: '#f59e0b' }}>+50 XP</strong> for supporting a local creator.
            </p>
            <div style={{ margin: '20px 0', padding: '14px', background: 'rgba(82,183,136,0.08)', borderRadius: 12, border: '1px solid rgba(82,183,136,0.15)' }}>
              {[
                ['Order ID',    `#KC-${Math.floor(Math.random() * 90000) + 10000}`, 'monospace'],
                ['Amount',      `KES ${pkg.price.toLocaleString()}`,                null       ],
                ['Delivery by', `${pkg.delivery} business days`,                    null       ],
              ].map(([k, v, ff]) => (
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.82rem', marginBottom: 6 }}>
                  <span style={{ color: '#5a7260' }}>{k}</span>
                  <span style={{ color: k === 'Amount' ? '#52b788' : '#e8f0eb', fontWeight: 700, fontFamily: ff ?? 'inherit' }}>{v}</span>
                </div>
              ))}
            </div>
    ...
    <div style={confirmStyles.overlay} onClick={onCancel}>
      <div style={confirmStyles.box} onClick={e => e.stopPropagation()}>
        ...
        <p style={{ fontSize: '0.7rem', color: '#5a7260', textAlign: 'center', marginTop: 10 }}>
          🔒 Protected by Chronicles Escrow — funds released on your approval
        </p>
      </div>
    </div>
  );
...
// ─── Shared inner content (reused by drawer and page modes) ───────────────────
  ...
  return (
    <>
      ...
      <div style={drawerStyles.header}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
            <span style={{ ...drawerStyles.islandTag, background: accent.bg, color: accent.color, border: `1px solid ${accent.color}33` }}>
      ...
      <div style={drawerStyles.sellerRow}>
        <div style={{ ...drawerStyles.sellerAvatar, background: `linear-gradient(135deg, ${accent.color}99, ${accent.color}44)` }}>
          {gig.sellerName?.[0]}
        </div>
        <div style={{ flex: 1 }}>
                ...
                >
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                      ...
                      {p.popular && (
                        <span style={{ fontSize: '0.6rem', fontWeight: 700, background: 'rgba(245,158,11,0.15)', color: '#f59e0b', padding: '1px 6px', borderRadius: 8, border: '1px solid rgba(245,158,11,0.25)' }}>POPULAR</span>
                      )}
                    </div>
                    <span style={{ fontSize: '0.75rem', color: '#5a7260' }}>{p.desc} · {p.delivery}d delivery</span>
                  </div>
                  <span style={{ fontSize: '0.95rem', fontWeight: 800, color: selectedPkg === i ? accent.color : '#e8f0eb', fontFamily: 'var(--font-display,"Playfair Display",serif)', whiteSpace: 'nowrap' }}>
      ...
      <div style={drawerStyles.footer}>
        ...
        <div style={{ display: 'flex', gap: 9, flexWrap: 'wrap' }}>
          ...
          {/* ── Hire Now → triggers addXP(50) via HireConfirmation ── */}
          <button
 ...
 * asPage=false (default) → slide-in drawer overlay
 * asPage=true → inline page section (used from App.jsx drill-down)
 */
export default function ServiceDetails({ gig, isOpen, onClose, asPage = false }) {
  ...
  // Resolve details from registry (falls back to 'default' entry)
  const details = GIG_DETAILS[gig?.id] ?? GIG_DETAILS.default;

  // Derive rank from seller XP using shared getRank helper
  const rank   = getRank(gig?.sellerXP ?? 1800);
  const accentMeta = ISLAND_META[gig?.island] ?? ISLAND_META.Beauty;
  const accent = { color: accentMeta.color, bg: accentMeta.bg };
  const pkg    = details.packages[selectedPkg];

  // Lock body scroll in drawer mode
  useEffect(() => {
    if (asPage) return;
  ...
  const content = (
    <ServiceContent
      gig={gig} details={details} accent={accent} rank={rank}
...
// ─── Style objects ────────────────────────────────────────────────────────────
const drawerStyles = {
  backdrop: { position: 'fixed', inset: 0, zIndex: 900, background: 'rgba(5,12,8,0.65)', backdropFilter: 'blur(4px)' },
  drawer: {
    position: 'fixed', top: 0, right: 0, bottom: 0,
    width: '100%', maxWidth: 480, background: '#0e1812',
    borderLeft: '1px solid rgba(82,183,136,0.14)', zIndex: 901,
    display: 'flex', flexDirection: 'column',
    animation: 'slideInRight 0.28s cubic-bezier(0.16,1,0.3,1)',
    boxShadow: '-24px 0 60px rgba(0,0,0,0.4)',
  },
  header:     { padding: '18px 20px 12px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: 12, alignItems: 'flex-start', flexShrink: 0 },
  title:      { fontFamily: 'var(--font-display,"Playfair Display",serif)', fontSize: '1.1rem', fontWeight: 700, color: '#e8f0eb', lineHeight: 1.35 },
  closeBtn:   { background: 'rgba(255,255,255,0.07)', border: 'none', borderRadius: '50%', width: 30, height: 30, cursor: 'pointer', color: '#a3b899', fontSize: '0.8rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  islandTag:  { fontSize: '0.65rem', fontWeight: 700, padding: '2px 9px', borderRadius: 20, letterSpacing: '0.07em' },
  sellerRow:  { padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid rgba(255,255,255,0.05)', flexShrink: 0, flexWrap: 'wrap' },
  sellerAvatar: { width: 42, height: 42, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, color: '#fff', flexShrink: 0 },
  rankBadge:  { fontSize: '0.65rem', fontWeight: 700, padding: '2px 8px', borderRadius: 20 },
  tabs:       { display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 },
  tab:        { flex: 1, padding: '11px 16px', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.82rem', transition: 'all 0.15s', fontFamily: 'var(--font-body,"DM Sans",sans-serif)' },
  body:       { flex: 1, overflowY: 'auto', padding: '20px' },
  description:  { fontSize: '0.85rem', color: '#a3b899', lineHeight: 1.75, marginBottom: 22, whiteSpace: 'pre-line' },
  sectionTitle: { fontSize: '0.72rem', fontWeight: 700, color: '#5a7260', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 },
  footer:     { padding: '14px 20px', borderTop: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(0,0,0,0.2)', flexShrink: 0, flexWrap: 'wrap', gap: 10 },
  msgBtn:     { padding: '9px 16px', borderRadius: 22, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: '#a3b899', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer' },
  hireBtn:    { padding: '9px 22px', borderRadius: 22, border: 'none', color: '#fff', fontSize: '0.88rem', fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 14px rgba(0,0,0,0.3)', transition: 'transform 0.15s, box-shadow 0.15s' },
};

const confirmStyles = {
  overlay:     { position: 'fixed', inset: 0, zIndex: 1100, background: 'rgba(5,12,8,0.75)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' },
  box:         { width: '100%', maxWidth: 440, background: '#111f17', border: '1px solid rgba(82,183,136,0.18)', borderRadius: 18, padding: '24px', boxShadow: '0 24px 60px rgba(0,0,0,0.5)', animation: 'slideDown 0.2s cubic-bezier(0.16,1,0.3,1)' },
  closeBtn:    { background: 'rgba(255,255,255,0.07)', border: 'none', borderRadius: '50%', width: 28, height: 28, cursor: 'pointer', color: '#a3b899', fontSize: '0.75rem', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  summaryCard: { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '14px', marginBottom: 16 },
  payBtn:      { padding: '12px 24px', borderRadius: 24, background: 'linear-gradient(135deg, #52b788, #3a7d44)', border: 'none', color: '#fff', fontSize: '0.9rem', fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 18px rgba(82,183,136,0.3)' },
};
